...
$(document).on("click", "#btnAgregar", null, function () {
    ...
    $('#myModalLabel').text("REGISTRAR UN Pedido");
    $.ajax({ url: "Pedido/RegistrarPedido", type: 'get' }).done(function (data) {
...
$(document).on("click", "#modal-dori-elim-ok", null, function () {
    ...
    $.post('Pedido/AtenderPedido', {
        CodigoPedido: $('#idHuespedEliminar').val()
    }, function (data) {
        if (data.Result == "OK") {
...
(function (sinisterNamespace, $, undefined) {
    ...
    $(function () {
        $(polizasTable).jtable({
            messages: { noDataAvailable: 'No se encontraron Pedidos' },
            title: 'Listado de Pedidos',
            paging: true,
            pageSize: 10,
            actions: {
                listAction: 'Pedido/ListarPedidos'
            },
            fields: {
                ROW: { title: 'N°', width: '3%', edit: false, },
                CodigoPedido: { title: 'N°', width: '3%', edit: false },
                Correlativo: { title: 'Correlativo', width: '1%' },
                DescriMenu: { title: 'Menu', width: '32%' },
                DescriEstado: { title: 'Estado', width: '5%' },
                Detalle: { title: 'Detalle', width: '7%' },
                ...
                Eliminar: {
                    ...
                    display: function (data) {
                        if (data.record.DescriEstado === "Nuevo") {
                            return '<a class="btn btn-default" onclick="eliminarHuesped(' + data.record.CodigoPedido + ')"> <i class="fa fa-check"></i> </a> ';
                        }
                      }
                }
